...
<script>
    $(function () {
        //初始化表单属性
        $("#vTable").jqGrid({
            url: "${path}/video/queryVideoPage",  //分页查询   page  rows  total recoreds  page  rows
            ...
            colNames: ['ID', '标题', '描述', '视频','封面', '上传时间', '用户Id', '类别id', '分组id'],
            colModel: [
                {name: 'id', width: 55},
                {name: 'title', editable: true, width: 90},
                {name: 'description', editable: true, width: 100},
                {
                    name: 'videoPath', editable: true, width: 200,height: 400, align: "center", edittype: "file",
                    formatter: function (cellvalue, options, rowObject) {
                        return "<video id='media' src='" + cellvalue + "' controls width='200px' heigt='400px' />";
                    }
                },
                {name:"coverPath",width: 100,edittype: "file",
                    formatter:function(cellvalue){
                        return "<img src='" + cellvalue + "' width='200px'>";
                    }
                },
